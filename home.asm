...
GetTrainerInformation::
	; IDDJY removed getting the trainer name since we overwrite it afterwards with the individual trainer name
	ld a, [wLinkState]
	and a
	jr nz, .linkBattle
...
.off
	...
	ld de, vChars2 + $620	; IDDJY increased by $20 since I removed 2 tiles from the tileset, so we need $20 less bytes
	ld bc, TextBoxGraphicsEnd - TextBoxGraphics
	ld a, BANK(TextBoxGraphics)
	jp FarCopyData2 ; if LCD is off, transfer all at once
.on
	ld de, TextBoxGraphics
	ld hl, vChars2 + $620	; IDDJY increased by $20 since I removed 2 tiles from the tileset, so we need $20 less bytes
...
ManualTextScroll::
	ld a, [wIsInBattle]
	cp 2
	jr z, .inLinkBattle						; IDDJY always skip waiting for key press during trainer battles
	call WaitForTextScrollButtonPress
	ld a, SFX_PRESS_AB
	jp PlaySound
.inLinkBattle
	ld c, 25								; IDDJY reduced from 65
	jp DelayFrames
