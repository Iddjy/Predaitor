...
.next2
	...
	call CopyData
	; IDDJY removed check to see if nicknaming is allowed
	ld hl, wPartyMonNicks
	ld a, [wMonDataLocation]		; IDDJY
	and $f							; IDDJY
	jr z, .next5					; IDDJY
	ld hl, wEnemyMonNicks			; IDDJY
.next5								; IDDJY
	ld a, [hNewPartyLength]
	dec a
	call SkipFixedLengthTextEntries
	; IDDJY removed nicknaming dialog
	ld a, [wcf91]					; IDDJY
	ld [wd11e], a					; IDDJY
	call GetMonName					; IDDJY
	ld d, h							; IDDJY
	ld e, l							; IDDJY
	ld hl, wcd6d					; IDDJY
	ld bc, NAME_LENGTH				; IDDJY
	call CopyData					; IDDJY set mon nickname to its species name
	ld hl, wPartyMons
	ld a, [wMonDataLocation]
	and $f
	jr z, .next3
